...
TOKEN_FILE_ID = os.path.join(STORAGE_PATH, "token_id.json")
EAT_FILE = os.path.join(STORAGE_PATH, "eat_tokens.txt")  # Google/EAT tokens
...
def generate_jwt_token(uid, password):
    ...
    try:
        ...
        app.logger.warning(f"MajorLogin failed for {uid}, trying external APIs...")
        return _fallback_external_api(str(uid), str(password))
    except Exception as e:
        app.logger.error(f"MajorLogin failed for {uid}: {e}")
        return _fallback_external_api(str(uid), str(password))

def _fallback_external_api(uid: str, password: str):
    """Fallback: generate token via API eksternal (kayak brx.py)"""
    import urllib.parse
    encoded_pw = urllib.parse.quote(password)
    apis = [
        f"https://get-ff-guest-jwt.vercel.app/get_jwt?uid={uid}&password={encoded_pw}",
        f"https://ffclient-ob53-acc-jwt.vercel.app/get_jwt?uid={uid}&password={encoded_pw}",
        f"https://xtytdtyj-jwt.up.railway.app/token?uid={uid}&password={encoded_pw}",
        f"https://ff-jwt-gen-api.lovable.app/api/public/token?uid={uid}&password={encoded_pw}",
        f"http://157.15.98.85:25565/generate-jwt?uid={uid}&password={encoded_pw}",
        f"http://87.232.72.68:3005/token?uid={uid}&password={encoded_pw}&key=dgop",
        f"https://api-ff-full.vercel.app/get_token?uid={uid}&password={encoded_pw}",
    ]
    for url in apis:
        try:
            ...
            if r.status_code == 200:
                j = r.json()
                token = j.get("jwt_token") or j.get("token") or j.get("jwt")
                if token and len(str(token)) > 20:
                    app.logger.info(f"Fallback API OK for {uid}")
                    return {
                        "uid": uid,
                        "token": token,
                        "region": "ID",
            ...
            continue
    app.logger.error(f"All APIs failed for {uid}")
    return None
...
def load_eat_tokens():
    """Load EAT/Google tokens from eat_tokens.txt"""
    tokens = []
    if not os.path.exists(EAT_FILE):
        return tokens
...
def eat_to_jwt(eat_url):
    """Konvert EAT token ke JWT"""
    import re
    match = re.search(r'[?&]eat=([^&]+)', eat_url)
    eat_token = match.group(1) if match else eat_url
    
    apis = [
        f"https://eat-api.thory.buzz/api?eatjwt={eat_token}",
        f"https://mg24-eat-to-access.vercel.app/access_token?eat_token={eat_token}",
        f"https://mafuuu-token-converter.onrender.com/token?eat={eat_token}",
        f"https://api-ff-full.vercel.app/eat_to_jwt?eat={eat_token}",
    ]
    for url in apis:
        try:
            import requests as req
            r = req.get(url, timeout=15, verify=False)
            if r.status_code == 200:
                j = r.json()
                token = j.get("token") or j.get("jwt") or j.get("jwt_token") or j.get("access_token")
                if token and len(str(token)) > 20:
                    return token
        except:
            continue
...
def refresh_google_tokens():
    """Refresh semua EAT/Google tokens"""
    eat_urls = load_eat_tokens()
    if not eat_urls:
        return
    
    results = []
    for eat_url in eat_urls:
        try:
            import re
            acc_match = re.search(r'account_id=(\d+)', eat_url)
            uid = acc_match.group(1) if acc_match else eat_url[:15]
            
            token = eat_to_jwt(eat_url)
            if token:
                results.append({
                    "uid": uid,
                    "token": token,
                ...
                app.logger.info(f"Google token OK: {uid}")
            else:
                app.logger.warning(f"Google token FAILED: {uid}")
        except Exception as e:
            app.logger.error(f"Google token error: {e}")
    
    if results:
        # Merge ke token_id.json
        existing = []
        if os.path.exists(TOKEN_FILE_ID):
            try:
                with open(TOKEN_FILE_ID, "r") as f:
                    existing = json.load(f)
            except:
                existing = []
        
        # Hapus google tokens lama, tambah yang baru
        existing = [t for t in existing if t.get("type") != "google"]
        existing.extend(results)
        save_tokens(TOKEN_FILE_ID, existing)
        app.logger.info(f"Saved {len(results)} Google tokens")
...
@app.route('/refresh_google', methods=['GET'])
def api_refresh_google():
    """Manual refresh Google/EAT tokens"""
    import threading
    def run():
        refresh_google_tokens()
    threading.Thread(target=run, daemon=False).start()
    return jsonify({'message': 'Google token refresh started', 'status': 'started'})
...
def api_token_status():
    ...
    return jsonify({
        ...
        "valid_tokens": len(valid_tokens),
            "guest_tokens": len([t for t in (valid_tokens or []) if t.get("type") != "google"]),
            "google_tokens": len([t for t in (valid_tokens or []) if t.get("type") == "google"]),
